...
read_hmmer <- function(file){
   ...
   colTypes <- data.table(
                  col_names=c('domain_name','domain_accession','query_name','query_accession',
                        'sequence_evalue','sequence_score','sequence_bias','best_domain_evalue',
                        'best_domain_score','best_domain_bis','domain_number_exp','domain_number_reg',
                        'domain_number_clu','domain_number_ov','domain_number_env','domain_number_dom',
                        'domain_number_rep','domain_number_inc'),
...
if(!is.null(arguments$customDbDir)){
   min_dependencies <- c(min_dependencies,c('hmmpress','hmmscan','gunzip'))
}
...
if(!arguments$useCustomDbOnly){
   ...
   scgTab <- data.table()
   write.log(paste0('useCustomDbOnly: skipping SCG prediction for default SCG sets: bacteria, archaea'),
             filename = logFile,type = 'cat')
}
...
if(!is.null(arguments$customDbDir)){
   ...
   for(scgSet in customSets){
      ...
      write.log(paste0('Annotating custom single copy genes: ',setName),filename = logFile,append = T,write_to_file = T,type = 'cat')
      ...
      # parse HMMER result
      hmmscan_tab <- read_hmmer(hmmscan_out) %>%
         data.table()
      if(nrow(hmmscan_tab)>0){
         ...
         write.log(paste0('No single copy genes found for set: ',setName),filename = logFile,append = T,write_to_file = T,type = 'warning')
      }
